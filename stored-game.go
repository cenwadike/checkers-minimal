...
import (
	...
	"cosmossdk.io/errors"
	"github.com/cenwadike/checkers/rules"
...
func (storedGame StoredGame) ParseGame() (game *rules.Game, err error) {
	...
	if errBoard != nil {
		return nil, errors.Wrapf(errBoard, ErrGameNotParsable.Error())
	}
	board.Turn = rules.StringPieces[storedGame.Turn].Player
	if board.Turn.Color == "" {
		return nil, errors.Wrapf(fmt.Errorf("turn: %s", storedGame.Turn), ErrGameNotParsable.Error())
	}
	return board, nil
}
